...
async fn setup() -> (Arc<TokenVerifier>, Arc<ApiKeyVerifier>, String, String) {
    ...
    let jwt_signer = Arc::new(JwtSigner::from_env()
        .expect("JWT_PRIVATE_KEY_PEM / JWT_PUBLIC_KEY_PEM must be set"));
    ...
    let tenant = tenant_store.create(&format!("bench-tenant-{now}"))
        .await
        .expect("Failed to create tenant");
    let sa = sa_store.create(&tenant.id, &format!("bench-sa-{now}"), None)
        .await
...
fn introspect_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("api_key_hot", |b| {
        b.to_async(&rt).iter(|| async {
            criterion::black_box(token_verifier_hot.verify(&api_key_hot).await)
        })
    });
    ...
    group.bench_function("jwt", |b| {
        b.to_async(&rt).iter(|| async {
            criterion::black_box(token_verifier_jwt.verify(&jwt_bench).await)
        })
    });
