...
async fn setup(pool: sqlx::PgPool) -> (PgTenantStore, PgServiceAccountStore, PgUserStore) {
    (
        ...
        PgServiceAccountStore::new(pool.clone()),
...
async fn admin_creates_api_key_and_introspects() {
    ...
    let (_, revoked) = sqlx::query_as::<_, (String, bool)>(
        "SELECT id, revoked_at IS NOT NULL FROM api_keys WHERE key_hash = $1",
    )
    .bind(&hash)
    .fetch_optional(&p)
    .await
...
async fn user_role_scope_and_oauth2() {
    ...
    let (_, pw) = sqlx::query_as::<_, (String, String)>(
...
async fn refresh_token_rotation() {
    ...
    let user = us
        .create(
            &tenant.id,
            &format!("rt-{}@t.com", unique_id("r")),
            &PasswordHasher::hash("x").unwrap(),
            None,
        )
        .await
        .unwrap();
