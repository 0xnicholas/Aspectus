...
当前 Pandaria 生态中，每个项目各自管理身份——Pandaria 用 HMAC token、Emerald 用 API Key、Constell 用 NextAuth。同一个租户在不同系统中无法关联。Aspectus 将这些分散的身份孤岛统一为一个可审计、可治理的单一身份层。

> **更新 2026-06-21**：Tavern 不再作为独立的生态项目存在。Tavern 的代码已合并入 Pandaria 作为子系统（位于 `pandaria/crates/tavern-*`），与 Pandaria 共用网关和身份层。Aspectus 侧不再为 Tavern 单独维护 `Project` 枚举值、scope、Service Token。
...
Aspectus 的授权边界止于**项目级访问控制**：「用户 U 属于租户 T，可以访问 Pandaria session、Constell agent」。数据级的细粒度授权（「用户 U 可以对 Customer 表 read 但不能 drop」）属于 Heirloom 的范畴。

| 层 | 谁管 | 粒度 | 例子 |
|----|------|------|------|
| 身份认证 | Aspectus | 「你是谁」 | 用户 U 属于租户 T，持有 API Key K |
| 项目访问 | Aspectus | 「你能进哪个系统」 | 用户 U 可以访问 Pandaria、Constell |
...
Aspectus 的主要消费者不是人类用户——是其他服务（Pandaria、Emerald、Constell、Tokencamp、Heirloom）。这意味着：
...
```json
{
  ...
  "scope": "pandaria:session:create pandaria:session:read constell:agent:publish",
  "token_type": "bearer",
  "exp": 1717000000
}
```
...
**理由**：细粒度 API Key 允许租户为不同项目签发不同权限的 Key。如果 Pandaria 的 Key 泄露，不影响 Constell 和 Emerald。
...
| **Role** | Scope 的命名集合。如 `agent-developer` = `[pandaria:session:*, constell:agent:*]`。 |
| **Client / Project** | 生态中的一个项目（Pandaria、Emerald、Constell、Tokencamp、Heirloom）。自省时作为 `client_id` 传入。 |
| **Token Introspection** |  RFC 7662 端点，验证 token 有效性并返回关联的 tenant/user/scopes。 |
| **Quota（配额）** | per-tenant 的资源限制配置。Aspectus 只存配置，不执行。 |

> 生态项目（Pandaria / Constell / Tokencamp / Heirloom / Emerald）如何接入 Aspectus，参见 [消费者接入指南](docs/consumer-integration.md)。
...
| 维度 | 推荐 | 理由 |
|------|------|------|
| 语言 | Rust | 与 Pandaria/Pawbun 一致，零成本自省性能 |
| 框架 | axum | 与 Pandaria 一致，生态内统一 |
...
- 契约测试：确保自省响应格式与 Pandaria/Constell 的期望一致。详见 [docs/contract-testing.md](docs/contract-testing.md) 与 `crates/aspectus-server/tests/http_tests/contract_test.rs`（基于 `insta` snapshot + `jsonschema`）
